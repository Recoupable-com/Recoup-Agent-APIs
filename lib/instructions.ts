export const instructions = {
  ...
  sort_fans_on_segments:
    "Analyze the provided comment data to determine which segment each user in the comment data belongs to. Usernames are included in the comment data, and you can analyze user comments to determine which segment each user belongs to. Analyze at least 100 users whose comments are reflected. Avoid repeating users too many times in the same segment. Segment names must be from the list of segments in the given context. The results are displayed as a list of usernames and their corresponding segment names in the following format: { [UserName]: [SegmentName] }.",
  get_fan_segments:
