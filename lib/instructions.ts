export const instructions = {
  ...
  sort_fans_on_segments:
    "Analyze the provided comment data to determine the segment to which each user belongs, as reflected in the comment data. User information is reflected in the comment data, and by analyzing the user's comments, it is possible to determine which segment each user belongs to. The results are displayed as a list of user names and their corresponding segment names, in the following format: { [UserName: SegmentName]: [SegmentName] }.",
  get_fan_segments:
